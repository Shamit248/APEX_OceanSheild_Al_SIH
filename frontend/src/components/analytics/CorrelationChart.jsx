  ...
  if (active && payload && payload.length) {
    return (
      <div className="p-2.5 rounded-lg bg-ocean-secondary border border-ocean-border shadow-xl text-xs font-mono">
        <p className="font-bold text-ocean-cyan mb-1">{label} Confidence</p>
        <p className="text-ocean-textPrimary">
          Candidate Vessels: <span className="font-bold text-ocean-cyan">{payload[0].value}</span>
        </p>
      </div>
    );
  }
...
const SCORE_COLORS = ['#EF4444', '#F59E0B', '#3B82F6', '#22D3EE', '#526D86'];
      ...
      <div className="flex items-center justify-between mb-4 pb-2 border-b border-ocean-border">
        <div>
          ...
          <p className="text-[11px] text-ocean-textMuted font-mono">
            Candidate vessel associations by probabilistic match bracket
          </p>
        </div>
      ...
      <div className="h-64 w-full">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
            ...
            <XAxis dataKey="bracket" stroke="#526D86" fontSize={11} tickLine={false} />
